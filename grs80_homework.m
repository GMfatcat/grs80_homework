...
for x=0:180
    for y =0:360
        U(x+1,y+1)=(GM/e)*atan(e/b)+((w^2)/3)*(a^2);
...
for x=0:180
        for y =0:360
            U10(x+1,y+1)=(GM/e)*atan(e/u)+((w^2)/2)*(a^2)*(Q10/Q)*((sin(x)^2) - 1/3)+((w^2)/2)*(u^2 + e^2)*(cos(x))^2;
...
for x = 0:179
        R(x+1)=re*(1+k*sin(x)^2)/sqrt(1-e2*sin(x)^2);
        R1(x+1) = re*(1+0.0052790414*sin(x)^2+0.0000232718*sin(x)^4+0.0000001262*sin(x)^6+0.0000000007*sin(x)^8);
        R2(x+1) = re*(1+0.0053024*sin(x)^2 -0.25*0.0000058*sin(2*x)^2);
